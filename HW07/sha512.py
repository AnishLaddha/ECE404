...
class hash():
    ...
    def _pad_input(self, inp_bv:BitVector) -> BitVector:
        ...
        p_list = [1]
        p_list+=([0]*(padding-1))
        inp_bv += BitVector(bitlist = p_list)
        last_128 = BitVector(intVal=(inp_bv.length() +128), size = 128)
        inp_bv += last_128

        return inp_bv
    
    def _process_block(self, block_bv:BitVector) -> BitVector:
    ...
    def _generate_msg_schedule(self, block_bv: BitVector) -> list:
        ...
        for i in range(17, 81):
            ...
            second = (self._sig_0(m_sched[i-15]))
            third = (m_sched[i-7]).intValue()
            fourth = (self._sig_1(m_sched[i-2])).intValue()
            total = (first + second + third + fourth) % pow(2,64)
    ...
    def _sig_0(self, bv: BitVector) -> BitVector:
        first = bv >> 1
        second = bv >> 8
        third = bv.shift_right(7)
        return first^second^third
    
    def _sig_1(self, bv: BitVector) -> BitVector:
        first = bv >> 19
        second = bv >> 61
        third = bv.shift_right(6)
